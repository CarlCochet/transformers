# coding=utf-8
# Copyright 2022 Michael Janner and Qiyang Li and Sergey Levine The HuggingFace Inc. team. All rights reserved.
#
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        output = torch.matmul(attn_weights, value)  # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        # [ B x T x embedding_dim ]
        output = (
            output.transpose(1, 2).contiguous().view(batch_size, sequence_length, embedding_dim)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        return outputs  # hidden_states, present, (attn_weights)
...
class TrajectoryTransformerModel(TrajectoryTransformerPreTrainedModel):
    ...
    def forward(
        ...
        >>> logits, loss = model(trajectories, **kwargs)
        ```
        """
